...
function displayMsgsList() {
	...
	for(var i = 0; i < msgsList.length; i++) {
		...
		msgsListDisplay += msgsList[i].renter + '</h4>';
		msgsListDisplay += '<p>';
		msgsListDisplay += msgsList[i].contactNum + '</p></div>';
	}
...
function displayMessages(index) {
	...
	// create the header
	$('#msgs-header h4').text(msgsList[index].renter)
	$('#msgs-header p').text(msgsList[index].contactNum)
